# План тестирования дипломного проекта приложения "Мобильный хоспис"
 
|        |        |
|---------|-----------|
|Дата проведения|___01.06.2024 - 21.09.2024___| 
|Студент| ___Некрасов Дмитрий *QAMID-60*___|



## Содержание
1.	Введение
    -	Основная информация
    -	Цель
2.	Область тестирования приложения
3.	Тест-план и стратегия тестирования
    -	Функциональное тестирование
    -	Процедура тестирования
    -	Отчеты об ошибках
4. Критерии начала тестирования
5. Результаты тестирования    
6.	Окружение
    -	Инструменты для тестирования на ПК
    -   Инструменты для тестирования на мобильном устройстве
7. Интервальная оценка с учетом рисков
8. Роли и обязанности
9.	Критерии качества
10.	Риски процесса тестирования




## 1.	Введение

### 	Основная информация

Документ описывает методы и подходы к тестированию, которые будут использоваться тестировщиками отдела тестирования, для тестирования  приложения.  Объект тестирования — проверка работоспособности мобильного приложения "Мобильный хоспис"
 
###	Цель

Тест-план проекта преследует следующие цели:
- Определить существующую информацию о проекте и программных компонентах, подлежащих тестированию.
- Описать стратегии тестирования, которые будут использоваться.
- Определить необходимые ресурсы для проведения работ по тестированию.
- Привести результаты тестирования в виде отчетов.

Все найденные ошибки будут отслеживаться с помощью баг-трекинговой системы.


## 2.	Область тестирования приложения
В объем тестирования приложения входит тестирование основного функционала приложения:
- авторизация в приложении;
- новостную сводку хосписа (просмотр новостей,  управление разделом новости);
- тематические цитаты;
- раздел с информацией о приложении (переход по ссылкам).

## 3.	Тест-план и стратегия тестирования
###	Функциональное тестирование
Цель функционального тестирования состоит в том, чтобы убедиться, что весь программный продукт работает в соответствии с требованиями и в приложении не появляется существенных ошибок. Программный продукт должен пройти все запланированные тесты.
Только в этом случае можно быть уверенным в его качестве.

|        |        |
|---------|-----------|
|Цель тестирования|	Обеспечение надлежащего качества целевой функциональности|
|Техника|Выполнение каждого сценария, используя допустимые и недопустимые данные:|  
|| *Ожидаемые результаты возникают при использовании достоверных данных*|
|| *Соответствующие сообщения об ошибках или предупреждения отображаются, когда используются неверные данные*|
||*Каждый пункт проверен*|
|Критерий приемки|Все дымовые тесты пройдены|
||Нет блокирующих багов|
||Все баги с высоким приоритетов поправлены|
||Ручные тесты пройдены|
||Автоматические тесты пройдены|

###	Процедура тестирования
Процедура тестирования предполагает фиксацию информации об обнаруженных ошибках программного обеспечения в багтрекинговой системе.

Основные проводимые типы тестирования:
 - Функциональное ручное тестирование;
 - Автоматическое unit-тестирование
  
Кейсы для ручного тестирования и для авто-тестов будут дублировать друг друга и охватывать весь базовый функционал приложения: базовые функции по работе с новостями, переходы по разделам приложения, авторизацмя в приложении, работа с цитатами и разделом "О нас". Так как в конце работы необходимо сравнить результаты по времязатратам ручного тестирования и авто-тесов, набор кейс должен быть примерно одинаковым. 

### Перечень автоматизируемых сценариев:
Тестирование функциональности экрана загрузки
Тестирование функциональности входа (экран авторизации), выхода из профиля
Тестирование функционала переходов на страницы и в разделы
Тестирование функционала развертывания, свертывания блоков и выпадающих списков
Тестирование функционала при заполнении форм в разделах "Creating News",  "Editing News".
Тестирование функционала создания, удаления, редактирования, отмены действий создания, удаления, редактирования, новостей, коментариев и претензий
Тестирование функционала поиска новостей
Тестирование функционала часов
Тестирование функцианала календаря

В рамках тест-плана не будут выполняться:
 - Тестирование безопасности
 - Тестирование нагрузки
 
###	Сообщения об ошибках
Отчеты об ошибках создаются для того, чтобы предоставить команде разработчиков и руководителю проекта исчерпывающую информацию об обнаруженных ошибках. Степень серьезности ошибок можно разделить на четыре категории:
-	Блокирующие дефекты — это сбой всей программной системы или критической подсистемы, тестирование не может быть выполнено после возникновения дефекта.
-	Критические дефекты — ошибки, которые также приводят к выходу из строя всей или части системы, но существуют некоторые альтернативы обработки, которые позволяют продолжить работу системы.
-	Дефекты среднего приоритета — не приводят к сбою, но приводят к тому, что система показывает неправильные, неполные или противоречивые результаты.
-	Дефекты низкого приоритета — это небольшие ошибки, которые не влияют на функциональность: опечатки, грамматические ошибки, неправильная терминология и т.д.

Баг репорт заводится по шаблону:
- Название баг-репорта.
-	Краткое описание, представляющее собой краткое описание проблемы.
-	Предусловия.
-	Шаги для воспроизведения ошибки;
-	Ожидаемый результат.
-	Фактический результат.
-	Дополнительная информация о дефекте в виде прикрепленных скриншотов или видеозаписей.
## 4. Критерии начала тестирования

 - определены требования к проекту, область применения, разработана стратегия тестирования
 - утвержден план тестирования, проанализированы требования, подготовлены тестовые данные
 - разработаны и утверждены тестовые наборы, рассмотрены и одобрены тестовые наборы, завершена разработка документации по тестированию

## 5. Результаты тестирования

 - файл Plan.md с описанием плана по проверке и автоматизации приложения;
 - файл Check с чек-листом проекта и отметками о пройденых и непройденых тестах в формате xlsx, xls, csv
  
 По результатам ручного тестирования: 
 - таблица Cases с тест-кейсами в формате xlsx, xls, csv 
  
 По результатам автоматического тестирования:
 - проект в Android Studio с авто-тестами;
 - файл README.md с описанием процедуры запуска авто-тестов;
 - allure-results.zip с отчётом о результатах прогона тестов (Allure);
 - файл Result.md с результатом сравнения времени проверки приложения по чек-листу руками и UI-тестами.

##	6. Окружение

###	Инструменты для тестирования на ПК
Тестовый стенд на базе ПК: 
- ОС Windows 10 Домашняя для одного языка, Версия	22H2, Сборка ОС	19045.4529
- Все необходимо программное обеспечение для стенда описано в таблице ниже

|        |        |
|---------|-----------|
|Система контроля версий*|	Git|
|Баг трекинговая система|	Issues (GitHub)|
|Тест-кейсы|	Excel (Microsoft Office)|
|Автотесты**| 	Android Studio, Espresso + Allure|
|Эмулятор|  Pixel 6a API 29|

  *Система контроля версия используется для фиксации всех изменений в продукте, сохранности всех версий и отката до нужного состояния при необходимости.

  **В качестве языка программирования для автотестов используется Java так как нашей платформой для тестирования является Espresso при Android Studio, которая поддерживает Java/Kotlin, что в дальнейшем избавит нас от допольнительных неудобств и конфликтов в процессе сборки нашего проекта. 

###	Инструменты для тестирования на мобильном устройстве
|        |        |
|---------|-----------|
|Баг трекинговая система|	Issues (GitHub)|
|Тест-кейсы|	Excel (Microsoft Office)|
|Ручное тестирование| 	Мобильное устройство Honor 10 (Android 10 (оболочка EMUI), Номер сборки 10.0.0.185, Процессор Kirin 970, ОЗУ 4 Гб, разрешение 2280х1080)

## 7. Интервальная оценка с учетом рисков
- подготовка тестовой документации - 72 часа (при отсутствии дополнительных рисков)
- проведение ручного тестирования - 12 часов  
- подготовка окружения - 2 часа (при отсутствии дополнительных рисков)
- написание автотестов, тестирование и отладка автотестов - 24 часа (при - отсутствии дополнительных рисков)
- формирование и анализ отчетов - 8 часов.

## 8. Роли и обязанности
|        |        |
|---------|-----------|
|Тестовая документация|	Некрасов Дмитрий|
|Ручное тестирование|	Некрасов Дмитрий|
|Автоматическое тестирование| 	Некрасов Дмитрий|
|Отчет о результатах тестирования| Некрасов Дмитрий|


##	9. Критерии качества
Продукт должен работать в соответствии с требованиями и техническим заданием (если таковые имеются). Продукт не должен содержать критических и блокирующих дефектов в окончательной версии проекта.
 
##	10. Риски процесса тестирования
Следующие проблемы могут повлиять на результаты тестирования:
- Изменения и модификации программного продукта, которые не были запланированы и не обсуждались заранее с командой тестирования.
-	Изменения в требованиях к программному обеспечению, которые не были предварительно обсуждены с командой тестирования.
-	Задержки в исправлении ошибок.
